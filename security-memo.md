...
- **rollout evidence.** three installed production-runtime waves completed through the stack-local public extension wrapper with zero `lease_manual_release`, zero `lease_wait_timed_out`, and zero failed releases. all expected writer results were present, and final lease statuses were `free` with `exists: false` after terminal completion. focused and required full checks passed. the lease is enabled at startup by default; set `OMP_GATE_MUTATION_LEASE` to `0`, `false`, or `off` before startup to disable it.
