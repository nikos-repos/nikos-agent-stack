...
* before editing a file, run `ncm list <file>` (directory rules plus the file's own contracts), then read the contracts on the declarations you call
* after adding or editing a contract, run `ncm check <path>`: grammar, label, the ceiling `until:`, id uniqueness. on OMP, `ncm` is a tool
