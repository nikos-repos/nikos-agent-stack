...
the extension lease is enabled at startup by default and applies only while gates are enabled. it uses the canonical git common directory plus the worktree root, so linked worktrees do not share one operation slot. it acquires the lease in the `tool_call` hook, immediately before one mutation-capable `write`, `edit`, or `bash` operation; omp runs that hook before its approval gate, so in an approval mode that prompts, the lease is held while the prompt waits. `task`, read-only tools, `xd://` device writes, and targets outside the bound worktree hold no lease. the gate does not re-register the builtin tools: overriding `write` would disable omp's `xd://` offload of rarely-used tool schemas.
