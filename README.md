...
| [`orchestration/index.ts`](orchestration/index.ts) | native omp commands, result tool, session recovery, and hidden-turn scheduling |
| [`orchestration/engine.ts`](orchestration/engine.ts) | deterministic process replay, effects, subprocesses, modes, and terminal behavior |
| [`orchestration/store.ts`](orchestration/store.ts) | sqlite event record, projections, fencing, profiles, blueprints, doctor, and repair |
| [`orchestration/cli.ts`](orchestration/cli.ts) | public `omnipotence` command-line interface |
